...
def fix_order(rule_dict: RuleDict, update: Update):
    if check_order(rule_dict, update):
        return update

    pages_checked = set()
    for index, page in enumerate(update.pages):
        pages_after = rule_dict.get_pages_after(page)
        if len(pages_after & pages_checked) > 0:
            indices = []
            for page_after in pages_after:
                try:
                    indices.append(update.pages.index(page_after))
                except:
                    pass

            newUpdate = update.clone()
            newUpdate.pages.pop(index)
            newUpdate.pages.insert(min(indices), page)
            return fix_order(rule_dict, newUpdate)
        pages_checked.add(page)
...
with open("./input.txt", "r") as file:
    ...
    for update in updateset:
        if (not check_order(rule_dict, update)):
            fixed_update = fix_order(rule_dict, update)
